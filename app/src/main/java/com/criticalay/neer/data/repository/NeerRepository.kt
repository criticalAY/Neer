...
class NeerRepository
    ...
    constructor(
        ...
        // ---- User ---- //
        fun getUserDetails(): Flow<User> = userDao.getUserDetails()
        ...
        fun getTargetAmount(): Flow<Int> = beverageDao.getTotalIntakeAmount()
        ...
        fun getWaterIntakesForToday(
            ...
            intakeDao.getWaterIntakesForToday(
                waterBeverageId = waterBeverageId,
                startOfDay = startDay,
                startOfNextDay = endDay,
            )

        fun getTodayTotalIntake(
            waterBeverageId: Long,
            startDay: LocalDateTime,
            endDay: LocalDateTime,
        ): Flow<Int> =
            intakeDao.getTotalWaterIntakeForToday(
                waterBeverageId = waterBeverageId,
                startDate = startDay,
                endDate = endDay,
            )
        ...
        fun getIntakeHistory(
            ...
            intakeDao.getWaterIntakesForDateRange(
                waterBeverageId = waterBeverageId,
                startDate = startDate,
                endDate = endDate,
            )
        ...
        // ---- Alarm ---- //
        suspend fun createAlarm(alarm: AlarmItem) = alarmDao.insertAlarm(alarmItem = alarm)

        suspend fun createAlarmReturningId(alarm: AlarmItem): Long = alarmDao.insertAlarmReturningId(alarmItem = alarm)
        ...
        suspend fun toggleAlarm(
            alarmId: Long,
            state: Boolean,
        ) = alarmDao.toggleAlarmState(alarmId = alarmId, state = state)
    }
